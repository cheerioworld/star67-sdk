...
namespace Star67.Avatar
{
  public class Star67AvatarLoader : PostprocessedAvatarLoaderBase
  {
    ...
    private static GameObject SpawnAvatarWithContentPolice(GameObject avatarPrefab, Transform parent, IAvatarDescriptor descriptor)
    {
      ...
      LoadFromLocalBeeAsync(string localBeePath, string password, BasisProgressReport progress, CancellationToken cancellationToken)
    {
        ...
        && connectorSize64 <= stream.Length - 8)
      {
        ...
        BasisBundleConnector connector = await BasisEncryptionToData.GenerateMetaFromBytes(password, connectorBytes, progress);
      ...
      BasisBundleConnector diskConnector = await BasisEncryptionToData.GenerateMetaFromBytes(password, diskConnectorBytes, progress);
    ...
    private static async Task<byte[]> ReadExactAsync(FileStream stream, int length, CancellationToken cancellationToken)
    {
      if (length < 0)
      {
    ...
    private static string BuildAvatarName(IAvatarDescriptor descriptor, BasisBundleConnector connector, string fallbackName)
    {
      string id =
